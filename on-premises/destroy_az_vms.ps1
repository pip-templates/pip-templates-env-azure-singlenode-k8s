...
# Delete all cb resources and deployment
for($i = 0; $i -lt $config.onprem_couchbase_vm_count; $i++) {
    $vmName = "$($config.onprem_couchbase_vm_resources_prefix)$i"
    Write-Host "Deleting virtual machine $vmName..."
    az vm delete -g $config.az_resource_group -n $vmName -y
    if ($LastExitCode -eq 0) {
        Write-Host "VM '$vmName' deleted."
...
# Delete vnets 
if ($config.onprem_k8s_vm_vnet -eq $config.onprem_couchbase_vm_vnet) {
    Write-Host "Deleting virtual network..."
    az network vnet delete -g $config.az_resource_group -n $config.onprem_k8s_vm_vnet
    if ($LastExitCode -eq 0) {
        Write-Host "VNET '$($config.onprem_k8s_vm_vnet)' deleted."
    }
} else {
    ...
    Write-Host "Deleting couchbase virtual network..."
    az network vnet delete -g $config.az_resource_group -n $config.onprem_couchbase_vm_vnet
    if ($LastExitCode -eq 0) {
        Write-Host "VNET '$($config.onprem_couchbase_vm_vnet)' deleted."
    }
}
